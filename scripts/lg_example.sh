...
function lg_example_home(){
    ...
    ./LG_B1_Client -k true -K 60 -M yes -f /dev/shm/looking-glass0 -a true -t "Android Home"
}
...
function lg_example_setting(){
    ...
    adb -s localhost:5555 shell am start -M yes -n com.android.settings/com.android.settings.homepage.SettingsHomepageActivity --display 1
...
function lg_example_documents(){
    ...
    adb connect localhost
    adb -s localhost:5555 shell am start -M yes -n com.android.documentsui/com.android.documentsui.files.FilesActivity --display 2
...
function lg_example_webview(){
    ...
    #display3 is Webview
    adb connect localhost
    adb -s localhost:5555 shell am start -M yes -n org.chromium.webview_shell/.WebViewBrowserActivity --display 3
